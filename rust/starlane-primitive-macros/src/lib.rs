        ...
        for variant in data.variants.clone() {
            ...
            if variant.fields.len() == 1 {
                    ...
                    Type::Path(path) => {
                        ...
                        if segment.ident == format_ident!("{}", "Box") {
                            ...
                            impl ToSubstance<#ty> for #ident {
                                fn to_substance(self) -> err::Result<#ty> {
                                    match self {
                                    Self::#variant_ident(val) => Ok(*val),
                                ...
                                fn to_substance_ref(&self) -> err::Result<&#ty> {
                                    match self {
                ...
                impl ToSubstance<()> for #ident {
                    fn to_substance(self) -> err::Result<()> {
                        match self {
                        Self::#variant_ident => Ok(()),
                        _ => Err(err!("expected Empty").into())
                        }
                    }
                     fn to_substance_ref(&self) -> err::Result<&()> {
                        match self {
                        Self::#variant_ident => Ok(&()),
                        _ => Err(err!("expected Empty").into())
                        }
...
pub fn mech_err(item: TokenStream) -> TokenStream {
    ...
    let rtn = quote! {
        ...
        impl MechErr for #ident {
            fn to_uni_err(self) -> starlane_space::err::Error {
               err!(self.to_string())
            }
        }
